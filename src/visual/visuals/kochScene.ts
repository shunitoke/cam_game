...
export class KochScene {
  ...
  private build(depth: number, warp: number, tilesX: number, tilesY: number) {
    // Start with an equilateral triangle (snowflake base)
    const s = 1.25;
    const h = (Math.sqrt(3) / 2) * s;
    const p0: P2 = { x: -s * 0.5, y: -h * 0.35 };
    const p1: P2 = { x: s * 0.5, y: -h * 0.35 };
    const p2: P2 = { x: 0, y: h * 0.65 };
    ...
    for (let d = 0; d < depth; d++) {
      ...
      for (const s0 of segs) {
        ...
        const pA: P2 = { x: ax, y: ay };
        const pB: P2 = { x: ax + vx / 3, y: ay + vy / 3 };
        const pD: P2 = { x: ax + (vx * 2) / 3, y: ay + (vy * 2) / 3 };
        const pE: P2 = { x: bx, y: by };

        // peak point
        const px = pB.x + (vx / 3) * Math.cos(theta) - (vy / 3) * Math.sin(theta);
        const py = pB.y + (vx / 3) * Math.sin(theta) + (vy / 3) * Math.cos(theta);
        const pC: P2 = { x: px, y: py };

        next.push({ a: pA, b: pB }, { a: pB, b: pC }, { a: pC, b: pD }, { a: pD, b: pE });
      }
      segs = next;
      if (this.safeMode && segs.length > 12000) break;
    }
    ...
    const warped: Array<{ ax: number; ay: number; bx: number; by: number }> = new Array(segs.length);
