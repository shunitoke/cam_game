...
export class WaveLabScene {
    ...
    reset() {
        this.t = 0;
        for (let i = 0; i < this.laneEnv.length; i++)
            this.laneEnv[i] = 0;
        this.burst = 0;
    }
    triggerBurst(amount) {
    ...
    update(control) {
        ...
        const spts = sg.getAttribute("position").array;
        const n = Math.min(256, fft?.length ?? 0);
        const fftLine = this.lines[7];
        if (fftLine)
            fftLine.position.y = (this.baseY[7] ?? -1.1) * yScale + yOffset;
        const fftLbl = this.labels[7];
        if (fftLbl)
            fftLbl.sprite.position.y = (this.baseY[7] ?? -1.1) * yScale + yOffset;
        for (let i = 0; i < 256; i++) {
            const u = i / 255;
        ...
        if (fftLbl)
            fftLbl.material.opacity = 0.55 + energy * 0.45;
        // partials editor
        ...
        const bpts = bars.getAttribute("position").array;
        const partials = edit?.target === "stab"
            ? ps
            : edit?.target === "lead"
                ? pl
        ...
        if (barsLine)
            barsLine.position.y = (this.baseY[8] ?? -1.33) * yScale + yOffset;
        const partialsLbl = this.labels[8];
        if (partialsLbl)
            partialsLbl.sprite.position.y = (this.baseY[8] ?? -1.33) * yScale + yOffset;
        barsMat.opacity = on ? 0.9 : 0.18;
        const tint = on && edit?.target === "stab"
            ? 0xffc8ff
            : on && edit?.target === "lead"
                ? 0xfff2a8
        ...
        if (partialsLbl)
            partialsLbl.material.opacity = on ? 0.95 : 0.25;
    }
    dispose() {
