...
export class CabinetScene {
    ...
    teapotA;
    teapotB;
    teapotHomeA = new THREE.Vector3();
    teapotHomeB = new THREE.Vector3();
    teapotVelA = new THREE.Vector3();
    teapotVelB = new THREE.Vector3();
    ...
    reset() {
        ...
        this.burst = 0;
        if (this.teapotA)
            this.teapotA.position.copy(this.teapotHomeA);
        if (this.teapotB)
            this.teapotB.position.copy(this.teapotHomeB);
        this.teapotVelA.set(0, 0, 0);
        this.teapotVelB.set(0, 0, 0);
    }
    triggerBurst(amount) {
    ...
    update(control) {
        ...
        if (this.teapotA && this.teapotB) {
            ...
            this.tmpV0.set(Math.sin(tt * 0.63) * 0.7 + Math.cos(tt * 0.21) * 0.3, Math.cos(tt * 0.51) * 0.6, Math.sin(tt * 0.37) * 0.4);
            this.tmpV1.set(Math.cos(tt * 0.58) * 0.6, Math.sin(tt * 0.44) * 0.7 + Math.cos(tt * 0.17) * 0.25, Math.cos(tt * 0.33) * 0.4);
        ...
        // Soft non-collision for cabinet objects (avoid visible intersections).
        {
            const items = [];
            if (this.plasmaMesh)
                items.push({ m: this.plasmaMesh, r: 0.62 });
            if (this.objA)
                items.push({ m: this.objA, r: 0.42 });
            if (this.objB && this.objB.visible)
                items.push({ m: this.objB, r: 0.38 });
            if (this.teapotA)
                items.push({ m: this.teapotA, r: 0.30 });
            if (this.teapotB && this.teapotB.visible)
                items.push({ m: this.teapotB, r: 0.30 });
            const clampCabinet = (p) => {
    ...
    dispose() {
        ...
        if (this.floorMesh?.geometry)
            this.floorMesh.geometry.dispose();
        if (this.floorMat?.dispose)
            this.floorMat.dispose();
        if (this.portalFrame) {
            this.portalFrame.traverse((o) => {
                if (o?.geometry?.dispose)
                    o.geometry.dispose();
                if (o?.material?.dispose)
                    o.material.dispose();
            });
        }
        if (this.portalRim?.geometry)
            this.portalRim.geometry.dispose();
        if (this.portalRim?.material?.dispose)
            this.portalRim.material.dispose();
        if (this.objA?.geometry)
            this.objA.geometry.dispose();
        if (this.objMatA?.dispose)
            this.objMatA.dispose();
        if (this.objB?.geometry)
            this.objB.geometry.dispose();
        if (this.objMatB?.dispose)
            this.objMatB.dispose();
        if (this.teapotGeom?.dispose)
            this.teapotGeom.dispose();
        if (this.teapotMat?.dispose)
            this.teapotMat.dispose();
        const tex = this.sineMat.uniforms.uTex.value;
