...
export class BmlTrafficScene {
    ...
    sprinkleSamplesBias(count, redBias01) {
        const n = this.grid.length;
        const c = Math.max(0, Math.floor(count));
    ...
    update(control) {
        ...
        if (Math.abs(delta) > 0.006) {
            ...
            if (delta > 0)
                this.sprinkleSamplesBias(amt, redBias);
            else
                this.removeSamples(amt);
        }
        this.blitToTexture();
    ...
    rebalanceDensity(targetDensity) {
        ...
        if (Math.abs(delta) < 0.01)
            return;
        if (delta > 0)
            this.sprinkleSamples(this.grid.length * clamp01(delta * 0.12));
        else
            this.removeSamples(this.grid.length * clamp01(-delta * 0.12));
    }
    dispose() {
