...
export class LloydScene {
  ...
  private points: any;
  private pointGeom: any;
  private pointMat: any;

  private lines: any;
  private lineGeom: any;
  private lineMat: any;
  ...
  private bestJ = new Int32Array(10);
  private bestD = new Float64Array(10);

  private posPts: Float32Array;
  private posLines: Float32Array;
  private lineSegs = 0;
  ...
  update(control: ControlState) {
    ...
    for (let i = 0; i < this.n; i++) {
      ...
      for (let j = 0; j < this.n; j++) {
        ...
        if (d >= this.bestD[kNN - 1]!) continue;
      ...
      for (let k = 0; k < kNN; k++) {
        ...
        if (this.lineSegs >= linesMax) break;
