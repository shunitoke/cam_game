...
function distPointSeg2(p: P2, a: P2, b: P2) {
  const vx = b.x - a.x;
  const vy = b.y - a.y;
  const wx = p.x - a.x;
  const wy = p.y - a.y;
...
function rdp(points: P2[], eps2: number): P2[] {
  ...
  if (idx >= 0 && maxD > eps2) {
    const left: P2[] = rdp(points.slice(0, idx + 1), eps2);
    const right: P2[] = rdp(points.slice(idx), eps2);
