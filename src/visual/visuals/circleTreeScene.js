...
export class CircleTreeScene {
    ...
    reset() {
        ...
        while (this.nodes.length < this.maxNodes) {
            ...
            // keep within bounds
            if (Math.abs(x) > 1.45 || Math.abs(y) > 0.95)
                continue;
        ...
        const u = new Array(limit);
        for (let i = 0; i < limit; i++)
            u[i] = new THREE.Vector4(0, 0, 0, 0);
        for (let i = 0; i < nCount; i++) {
            const n = this.nodes[i];
            u[i] = new THREE.Vector4(n.x, n.y, n.r, n.depth);
        }
    ...
    update(control) {
        ...
        const limit = 256;
        const arr = this.mat.uniforms.uNodes.value;
        const nCount = Math.min(limit, this.mat.uniforms.uCount.value | 0);
