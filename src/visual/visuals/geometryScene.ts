...
export class GeometryScene {
  ...
  private lineMat = new THREE.MeshBasicMaterial({ color: 0x9bf2ff, wireframe: true, transparent: true, opacity: 0.65 });
  private bg = new THREE.Color(0x05060a);
  ...
  update(control: ControlState) {
    ...
    for (let i = 0; i < this.rings.length; i++) {
      ...
      if (r.position.z > 1.2) r.position.z = -18.2;
