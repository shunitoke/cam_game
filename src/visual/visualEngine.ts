...
type SceneImpl = {
  ...
  dispose: () => void;
  getScene: () => any;
  onResize?: (camera: any) => void;
  triggerBurst?: (amount: number) => void;
};
...
export class VisualEngine {
  ...
  private rt: any;
  private postScene: any;
  private postCamera: any;
  private postQuad: any;
  private postMat: any;

  private catAmount = 0;
  private catVariation = 0;
  private postTime = 0;
...
uniform float uStatic; // TV static noise amount during transitions
    ...
    for (const sc of [particles, geo, cabinet, logPolar, warp, cellular, sea, coyote, nikos, drone, wave, phys, quasi, ascii, bif, lloyd, rrt, arbor, koch, dla, bosWarp, kalei, metaballs] as any[]) {
  ...
  update(control: ControlState) {
    ...
    const bp = (control as any).beatPulse ?? 0;
    ...
    (this.postMat.uniforms.uAmount.value as number) = a;
    ...
    (this.postMat.uniforms.uIters.value as number) = Math.max(0, Math.min(8, baseIters + jitter));
    (this.postMat.uniforms.uTime.value as number) = this.postTime;
    // TV static during transitions
    (this.postMat.uniforms.uStatic.value as number) = a * 0.8; // scale with catAmount
