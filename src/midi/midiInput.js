...
export class MidiInput {
    ...
    async start() {
        ...
        try {
            ...
            this.access = null;
            this.lastError = e?.message ? String(e.message) : "MIDI access denied";
        }
    }
    stop() {
    ...
    getStatus() {
        ...
        if (this.access) {
            ...
            const shouldRecover = this.inputs.length > 0 &&
                scAt > 0 &&
                now - scAt > graceMs &&
                msgAt < scAt &&
            ...
            const stuckAtZero = this.inputs.length === 0 &&
                scAt > 0 &&
    ...
    refreshInputs() {
        ...
        for (const input of this.inputs) {
            const handler = (ev) => this.onMessage(ev);
