...
class DroneProcessor extends AudioWorkletProcessor {
  ...
  private padPhase1 = 0;
  private padPhase2 = 0;
  private padPhases: [number, number, number, number] = [0, 0, 0, 0];
  private padChorusPhase = 0;
  private padEnv = 0;
  private padGate = 0;
  private padTarget = 0;
  private padGain = 0;
  private padFreq = 220;
  private padBright = 0.4;
  ...
  process(_inputs: Float32Array[][], outputs: Float32Array[][]) {
    ...
    for (let i = 0; i < ch0.length; i++) {
      ...
      const polyBlep = (phase: number, dt: number) => {
        if (phase < dt) {
          const t = phase / dt;
          return t + t - t * t - 1;
        } else if (phase > 1 - dt) {
      ...
      if (this.padChorusPhase >= 1) this.padChorusPhase -= 1;
      ...
      for (let v = 0; v < 4; v++) {
        ...
        if (this.padPhases[v] >= 1) this.padPhases[v] -= 1;
      ...
      if (this.mode === "performance") {
        ...
        if (Math.random() < this.leadProb) this.leadGate = 1;
      }
      ...
      if (this.leadPhase >= 1) this.leadPhase -= 1;
